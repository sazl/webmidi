...
let init =
  let worker_init (e : WebWorkers.MessageEvent.t) =
    ...
    then (
        Js.log("starting");
        interval := Js.Nullable.return (Js.Global.setInterval tick interval_duration)
    )
    else (
      match Js.toOption !interval with
      | Some interval_id -> Js.Global.clearInterval interval_id
      | _ -> ()
    )
  in
